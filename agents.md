...
`repo update`  
Codex führt ausschließlich serielle Git-Kommandos aus. Zuerst wird das lokale Repository mit `git fetch --all --prune` aktualisiert. Danach prüft Codex die lokalen Branches und fragt ausdrücklich nach, ob alle lokalen Branches, deren Remote-Gegenstück oder Upstream nach dem Fetch vorhanden ist, seriell mit `git pull --ff-only` aktualisiert werden sollen. Ohne diese Bestätigung werden keine Pulls ausgeführt. Vor einem Pull-Lauf muss Codex prüfen, ob der Arbeitsbaum sauber ist; bei uncommitteten Änderungen, Divergenzen, Pull-Konflikten oder nicht fast-forward-fähigen Branches bricht Codex den Pull-Teil kontrolliert ab und dokumentiert den Blocker. Anschließend liest Codex für den Repo-Aktivitätsreport zusätzlich `docs/wiki/journal/README.md` und die für die neuen oder aktuellsten Remote-Commits relevanten Journaleinträge unter `docs/wiki/journal/`. Wenn Pulls ausgeführt wurden und dadurch neue Journaleinträge lokal verfügbar geworden sind, erfolgt diese Journallektüre nach den Pulls. Der Repo-Aktivitätsreport führt Git- und Journalinformationen zusammen: aktuellster Remote-Branch nach letztem Commit, Zeitpunkt des letzten Commits, soweit zuverlässig verfügbarer Push-Zeitpunkt oder andernfalls der Hinweis, dass normales Git den tatsächlichen Push-Zeitpunkt nicht zuverlässig lokal speichert, Liste aller lokalen Branches ohne vorhandenes Remote-Gegenstück oder Upstream, zuordenbare Journaleinträge, belegte Arbeitsschwerpunkte, dokumentierte Tests oder Verifikationen sowie offene Punkte, soweit sie im Journal genannt sind. Codex trennt dabei klar zwischen durch Commits belegten Informationen, durch Journal belegten Informationen und eigenen Zuordnungen. Fehlen passende Journaleinträge oder passen Commit- und Journalinformationen nicht eindeutig zusammen, dokumentiert Codex diese Lücke knapp und erfindet keine fachlichen Zusammenhänge.
