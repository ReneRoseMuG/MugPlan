...
Das autoritative Rollenmodell ist serverseitig und wird über die Datenbankbeziehungen `users -> roles` abgeleitet. Der Request‑Kontext wird im Ist‑Stand über Session‑Auth aufgebaut: `sessionAuth` (Session-Cookie), `requireSessionUser` (setzt `req.userId` aus der Session) und `resolveUserRole` (setzt `req.userContext = { userId, roleCode, roleKey }`). Client‑seitige Header sind keine Rollen‑ oder Autorisierungsquelle.
...
Das autoritative Rollenmodell ist serverseitig über `users -> roles` definiert und wird als `req.userContext` in den API‑Pfad injiziert. Der Ist‑Stand nutzt Login-/Setup‑Endpunkte mit Session‑Cookie und schützt nicht‑öffentliche API‑Routen über `requireSessionUser` plus `resolveUserRole`. Ein externes SSO/OIDC‑Modell ist nicht Teil des aktuellen Stands.
...
Der autoritative Rollenwert kommt aus `req.userContext.roleKey`. Dieser Kontext wird serverseitig über `sessionAuth`, `requireSessionUser` und `resolveUserRole` aufgebaut. Client‑Header sind keine Rollenquelle.
...
Validierungsfehler entstehen, wenn Request‑Body oder Query nicht dem Zod‑Contract entsprechen. Im Ist‑Stand antworten die meisten Controller auf Zod‑Fehler mit HTTP 422 und `{ code: "VALIDATION_ERROR" }`. Der Helper `handleZodError` bleibt für einzelne Endpunkte im Einsatz und antwortet dort mit HTTP 400 sowie `{ message, field }`.

Fachfehler entstehen, wenn der Request formal korrekt ist, aber gegen Regeln verstößt, insbesondere im Terminbereich. Im Appointment‑Controller wird hierfür ein domänenspezifischer Error‑Typ aus dem Service erkannt (AppointmentError) und als `{ code }` (bei Konflikten optional mit Zusatzfeldern wie `conflictEmployees`) beantwortet. Fuer die Sperrlogik ist im Ist‑Stand der maschinenlesbare Code `LOCK_VIOLATION` nachweisbar; fuer die serverseitige Ueberschneidungsblockierung bei Mitarbeiterzuweisungen ist der Nachweis im aktuellen Stand nicht eindeutig abgeschlossen.
...
## D8. User-Kontext und aktueller Ist-Stand

Der Ist‑Stand nutzt Session‑Auth: `sessionAuth` verwaltet das Session-Cookie, `requireSessionUser` setzt `req.userId` aus der Session und `resolveUserRole` erzeugt `req.userContext`. Für Settings wird damit kein statischer System-User mehr verwendet.

Die Rollenauflösung bleibt autoritativ serverseitig DB-basiert (`users -> roles`) und wird nicht aus Client-Headern abgeleitet.
...
Die API-Middleware-Reihenfolge ist im Ist-Stand:

1. `sessionAuth`
2. `setupGate`
3. `authRoutes` (Setup/Login/QuickLogin/Logout/Health)
4. `requireSessionUser`
5. `resolveUserRole`
6. fachliche Route-Handler

`requireSessionUser` liefert den technischen Benutzerkontext (`req.userId`) aus der Session.

`resolveUserRole` laedt User und Rolle aus der DB und erzeugt den kanonischen Rollenkey (`LESER`, `DISPONENT`, `ADMIN`) im Request.

### G3. Betriebsmodus mit Session-Auth

Der Ist-Stand nutzt ein Session-basiertes Auth-Modell mit Setup- und Login-Endpunkten:

- Setup-Guard vor geschuetzten API-Pfaden (`setupGate`)
- Login setzt `req.session.userId`
- geschuetzte Endpunkte verlangen gueltige Session (`requireSessionUser`)
- Rollen bleiben DB-basiert (`users -> roles`) ueber `resolveUserRole`
...
- HTTP `403`
- maschinenlesbarer Code `LOCK_VIOLATION`
...
- Angepasste Abschnitte:
  - Rolleninformation als Request-Kontext (`sessionAuth`/`requireSessionUser`/`resolveUserRole` statt statischer System-User-Injektion).
  - Auth-/Rollen-Risikoeinschätzung im Ist-Stand (Session-basiert, kein externes SSO/OIDC im Scope).
  - FT14-Middlewarekette und Betriebsmodus auf den aktuellen Session-/Setup-Flow korrigiert.
  - Fehler-/Lock-Code in der Terminlogik auf `LOCK_VIOLATION` präzisiert.
- Strukturelle Ergänzungen:
  - Keine neuen Kapitelstrukturen; bestehende Kapitel wurden punktuell aktualisiert.
- Festgestellte dokumentierte Abweichungen:
  - Frühere Architekturtexte zu `SETTINGS_USER_ID`/`attachRequestUserContext` waren nicht mehr codekonform und wurden redaktionell synchronisiert.
