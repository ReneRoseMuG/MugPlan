...
| `tests/integration/server/ft07.backup-and-caldav.integration.test.ts` | Integration test | `DELETE FROM backup_log` | via `db.execute` | expliziter destructive guard vor delete | open |
...
| `script/sql/reset_absolute_state.sql` | manuell SQL | drop + recreate schema | SQL client | SQL guard block + Test-DB-Namensregel + Host/Port-Plausi | open |
