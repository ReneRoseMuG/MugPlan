...
Dieses Feature ermöglicht den kontrollierten Import von Stammdaten in das System. Der Import dient zwei Zwecken, nämlich erstens dem Einspielen vorbereiteter Stammdaten aus CSV-Dateien und zweitens dem Bulk-Import aus PDF-Dokumenten über die vorhandene Extraktionsfunktion mit anschließender Prüfung in einer generierten Import-Tabelle. Das Feature ist ausschließlich für Administratoren bestimmt und verfolgt das Ziel, neue Datensätze sicher, nachvollziehbar und ohne unkontrollierte Überschreibungen zu übernehmen.
...
CSV-Dateien werden direkt eingelesen, als Vorschläge in einer Importtabelle abgelegt, validiert, korrigiert und selektiv übernommen. Siehe UC 23/01.

### Szenario 2: PDF-Bulk-Import (zweistufig)

### Stufe 1 – Kundenextraktion mit Fehlerprotokollierung

- Administrator lädt eine oder mehrere PDF-Aufträge hoch
- Das System verarbeitet jedes PDF über die Extraktionsfunktion (FT 21)
...
- Das System erzeugt eine **Fehlerspalte** pro Zeile und dokumentiert:
    ...
    - Fehlender Name
    - Unvollständige Adress-Daten
    - Ungültige Formate (z.B. Telefon)
...
### Stufe 2 – Dokumentextraktion mit Kundenzuordnung

**Vorbedingung:** Stufe 1 wurde durchgeführt, Kunden wurden via CSV-Import in das System übernommen.

- Administrator startet Stufe 2 und wählt die PDFs aus (oder System nutzt persistierte PDF-Liste aus Stufe 1)
- Das System verarbeitet jedes PDF erneut über die Dokumentextraktionsfunktion (FT 21)
- Pro PDF extrahiert das System: **Kundennummer, Kundenname, Saunamodell, Artikelliste**
- **Matching:** Das System prüft – existiert die extrahierte Kundennummer in der Kundentabelle?
    - **Ja:** System ordnet den Kundendatensatz zu
    - **Nein:** System legt für dieses PDF **kein Projekt an** und protokolliert die Warnung
- Für erfolgreich gematchte PDFs legt das System ein neues **Projekt** an:
    - **Titel:** Saunamodell (extrahiert)
    - **Beschreibung:** Artikelliste (als HTML-formatierte Auflistung)
    - **Kunde:** Der gematchte Kundendatensatz
- **Ergebnis:** Übersichtsliste aller erzeugten Projekte mit korrekter Kundenzuordnung, unmittelbar dispositionierbar

## FT 23 Regeln & Randbedingungen

**Allgemeine Regeln:**

- Der Import ist nur für Administratoren verfügbar
...
- Wenn ein Datensatz als Duplikat erkannt wird, darf er nicht übernommen werden

### CSV-Kundenimport – Fehlertoleranz
...
- Fehlende oder ungenaue Daten in **optionalen Feldern** blockieren **nicht** die Übernahme
- Diese Zeilen werden mit einer **Warnung** gekennzeichnet (Status: "übernehmbar mit Warnung")
- Der Administrator kann diese Zeilen vor Übernahme korrigieren oder ausschließen
- Fehlende **Pflichtfelder** führen zu Status "Konflikt", die Zeile kann nicht übernommen werden

### PDF-Bulk Stufe 2 – Dokumentenzuordnung

**Matching-Logik:**

- Extrahierte Kundennummer wird **exakt** gegen die Kundennummer aus Stufe 1 gematchet
- **Exaktes Matching nur** – kein Fuzzy-Matching, kein Fallback auf Name oder Adresse
- Wenn keine Übereinstimmung: Projekt wird nicht angelegt, Admin wird aufgefordert, Kundenanlage zu überprüfen

### Allgemeine Validierungsregeln

- Das System muss vor der Übernahme einen Konfliktstatus pro Zeile anzeigen, damit der Administrator die Ursache erkennt und die Zeile entweder korrigieren oder ausschließen kann
- Das System muss eine Übernahme nur dann erlauben, wenn alle zu übernehmenden Zeilen **Pflichtfelder vollständig** enthalten
- Das System muss jeden Importlauf mit Zeitstempel, Benutzer, Anzahl übernommener Zeilen und Anzahl Konflikte protokollieren

## FT 23 Use Cases

### UC 23/01 – Mitarbeiter aus CSV importieren
