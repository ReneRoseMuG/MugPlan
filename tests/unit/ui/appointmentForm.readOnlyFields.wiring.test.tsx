 ...
 * Test Scope:
 *
 * Feature: FT01 - Kontextuelle Terminanlage mit Feldsperren
 * Use Case: UC Readonly-Project/Customer im AppointmentForm
 *
 * Abgedeckte Regeln:
 * - AppointmentForm akzeptiert `readOnlyFields` als kontextuelle Sperre.
 * - Projekt-Slot wird bei gesetzter Projektsperre readonly gerendert.
 * - Kunden-Slot wird bei gesetzter Kundensperre readonly gerendert.
 *
 * Fehlerfaelle:
 * - Projektwechsel bleibt trotz Kontextsperre interaktiv.
 * - Kundenslot ignoriert die explizite Sperre.
 *
 * Ziel:
 * Die kontextuelle Feldsperre im Terminformular ueber gerenderte Slot-Zustaende statt Quelltext-Strings absichern.
 */
...
function buildQueryResult(queryKey: unknown): { data: unknown; isLoading: boolean } {
  ...
  if (key === "/api/tours" || key === "/api/teams" || key === "/api/employees") {
...
describe("FT01 appointment form readonly fields", () => {
  beforeEach(() => {
  ...
  it("renders the project slot as readonly when the project field is context-locked", () => {
    renderToStaticMarkup(<AppointmentForm projectId={11} readOnlyFields={["project"]} />);

    const projectSlot = getSlot("slot-project-relation");
    const customerSlot = getSlot("slot-customer-relation");

    expect(projectSlot.state).toBe("readonly");
    expect(projectSlot.onAdd).toBeUndefined();
    expect(projectSlot.onRemove).toBeUndefined();
    expect(customerSlot.state).toBe("readonly");
  });

  it("renders the customer slot as readonly when the customer field is explicitly locked", () => {
    renderToStaticMarkup(<AppointmentForm readOnlyFields={["customer"]} />);
    ...
    expect(customerSlot.state).toBe("readonly");
    expect(customerSlot.onAdd).toBeUndefined();
    expect(customerSlot.onRemove).toBeUndefined();
  });
});
