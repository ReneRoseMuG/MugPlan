 ...
 *
 * Abgedeckte Regeln:
 * - Die globalen FT31-Settings fuer alle Termine, Horizont und Mindestzahl sind im Registry-Modell vorhanden.
...
describe("settings registry: monitoring", () => {
  it("defines the global TR-01 settings with stable defaults", () => {
    expect(userSettingsRegistry.monitoringTr01AllAppointments.key).toBe("monitoring.tr01.allAppointments");
    expect(userSettingsRegistry.monitoringTr01AllAppointments.defaultValue).toBe(false);
    expect(userSettingsRegistry.monitoringTr01AllAppointments.allowedScopes).toEqual(["GLOBAL"]);
