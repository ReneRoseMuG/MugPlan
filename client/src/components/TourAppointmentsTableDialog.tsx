...
interface TourAppointmentsTableDialogProps {
  open: boolean;
...
export function TourAppointmentsTableDialog({
  open,
  onOpenChange,
  tourId,
  tourName,
}: TourAppointmentsTableDialogProps) {
  const fromDate = getBerlinTodayDateString();
  const queryUrl = tourId ? `/api/tours/${tourId}/current-appointments?fromDate=${fromDate}` : null;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[100dvw] h-[100dvh] max-w-none p-0 overflow-hidden rounded-none sm:w-[95vw] sm:h-[85vh] sm:max-w-6xl sm:rounded-lg">
        <div className="h-full flex flex-col p-6 gap-4">
          <DialogHeader>
            <DialogTitle>
              Termine {tourName ? `- ${tourName}` : ""} ({sortedAppointments.length})
            </DialogTitle>
          </DialogHeader>
          <div className="flex-1 min-h-0">
            <ListLayout
              ...
              viewModeKey="tourAppointmentsDialog"
              isLoading={isLoading}
              showCloseButton={false}
              contentSlot={(
                <TableView
                  testId="table-tour-appointments-dialog"
