...
export async function resolveUserRole(req: Request, _res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!userWithRole) {
      next(new Error(`Cannot resolve role for configured user ${userId}`));
      return;
    }

    if (!userWithRole.isActive) {
      next(new Error(`Configured user ${userId} is inactive`));
      return;
    }

    if (!userWithRole.roleCode) {
      next(new Error(`Configured user ${userId} has no valid roleCode`));
      return;
    }
