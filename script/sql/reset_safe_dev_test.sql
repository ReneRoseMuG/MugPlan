-- WARNUNG: SAFE RESET NUR FUER DEV/TEST - NICHT BLIND AUF PROD AUSFUEHREN.
-- WARNUNG: DESTRUKTIV - LOESCHT BESTEHENDE DATEN IM AKTUELL AKTIVEN SCHEMA.

-- ============================================================================
-- BLOCK: GUARD (DEV/TEST ONLY)
-- ============================================================================
SELECT DATABASE() AS active_database_before_guard;

DELIMITER $$
DROP PROCEDURE IF EXISTS `__guard_safe_reset_dev_test`$$
CREATE PROCEDURE `__guard_safe_reset_dev_test`()
BEGIN
  ...
  IF v_db_name = '' THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'SAFE RESET ABGEBROCHEN: Keine aktive Datenbank.';
  END IF;

  IF v_db_name = 'mysql_rvtagh' THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'SAFE RESET ABGEBROCHEN: Produktionsdatenbank mysql_rvtagh erkannt.';
  END IF;

  IF v_db_name NOT IN ('mysql_ak9036', 'mysql_jefm0k') THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'SAFE RESET ABGEBROCHEN: Nur mysql_ak9036 (DEV) oder mysql_jefm0k (TEST) erlaubt.';
  END IF;
END$$
CALL `__guard_safe_reset_dev_test`()$$
DROP PROCEDURE IF EXISTS `__guard_safe_reset_dev_test`$$
DELIMITER ;
