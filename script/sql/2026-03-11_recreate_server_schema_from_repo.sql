...
CREATE TABLE `component_specifications` (
  ...
  `created_at` timestamp NOT NULL DEFAULT (now()),
  `updated_at` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT `component_specifications_id` PRIMARY KEY(`id`)
);
...
CREATE TABLE `project_order_items` (
  ...
  CONSTRAINT `chk_project_order_items_relation_consistent` CHECK(((`project_order_items`.`product_id` IS NOT NULL AND `project_order_items`.`component_id` IS NULL) OR (`project_order_items`.`product_id` IS NULL AND `project_order_items`.`component_id` IS NOT NULL) OR (`project_order_items`.`product_id` IS NULL AND `project_order_items`.`component_id` IS NULL AND `project_order_items`.`description` IS NOT NULL)) AND (`project_order_items`.`specification_id` IS NULL OR `project_order_items`.`component_id` IS NOT NULL))
);
...
ALTER TABLE `component_specifications` ADD CONSTRAINT `component_specifications_component_id_components_id_fk` FOREIGN KEY (`component_id`) REFERENCES `components`(`id`) ON DELETE cascade ON UPDATE no action;
...
ALTER TABLE `project_order_items` ADD CONSTRAINT `project_order_items_specification_id_component_specifications_id_fk` FOREIGN KEY (`specification_id`) REFERENCES `component_specifications`(`id`) ON DELETE restrict ON UPDATE no action;
...
CREATE UNIQUE INDEX `uq_component_specifications_value` ON `component_specifications` (`component_id`,`spec_name`,`spec_value`(191));
CREATE INDEX `idx_component_specifications_component` ON `component_specifications` (`component_id`);
